...
export default async function NewProposalPage() {
  ...
  return (
    <PageShell className="max-w-[720px]">
      <Link href="/dashboard/proposals" className="mb-4 inline-flex items-center gap-1 text-[12px] text-muted hover:text-ink">
        <ChevronLeft className="size-4" /> Proposals
      </Link>
      <PageHeader title="New proposal" subtitle="Sent to the client and tracked here" />
      ...
      <Card className="mt-6 gap-5 p-6">
        <form action={createProposalAction} className="space-y-5">
          <div>
            ...
            <input name="title" required placeholder="e.g. Full-Service Retainer" className={`mt-1.5 ${field}`} />
          </div>
          ...
          <div>
            ...
            <textarea name="notes" rows={3} placeholder="Scope, terms, anything else…" className={`mt-1.5 ${field} h-auto py-2`} />
          </div>
