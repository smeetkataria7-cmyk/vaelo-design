...
import { LineItems } from "@/components/app/line-items";
...
export default async function NewInvoicePage() {
  ...
  return (
    <PageShell className="max-w-[720px]">
      <Link href="/dashboard/invoicing" className="mb-4 inline-flex items-center gap-1 text-[12px] text-muted hover:text-ink">
        <ChevronLeft className="size-4" /> Invoicing
      </Link>
      <PageHeader title="New invoice" subtitle="INR · GST applied on the subtotal" />
      ...
      <Card className="mt-6 gap-5 p-6">
        <form action={createInvoiceAction} className="space-y-5">
          ...
          <div>
            <label className="eyebrow !text-muted-2">Line items</label>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label className="eyebrow !text-muted-2">GST %</label>
              <input name="gst_percent" defaultValue="18" inputMode="decimal" className={`mt-1.5 ${field}`} />
            </div>
            <div>
              <label className="eyebrow !text-muted-2">Due date</label>
              <input name="due_date" type="date" className={`mt-1.5 ${field}`} />
            </div>
          </div>
          <div>
            <label className="eyebrow !text-muted-2">Notes / memo</label>
            <input name="notes" placeholder="e.g. June retainer" className={`mt-1.5 ${field}`} />
          </div>
          <div className="flex justify-end gap-2">
            <Button asChild variant="secondary" size="sm">
              <Link href="/dashboard/invoicing">Cancel</Link>
            </Button>
